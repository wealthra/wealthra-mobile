...
const IncomeScreen: React.FC<IncomeScreenProps> = ({ isDarkMode, onToggleTheme, navigation }) => {
   ...
   const handleAddIncome = async (newIncome: { name: string; amount: number; method: string; isRecurring: boolean }) => {
      try {
            ...
            { text: t("alert.cancel"), style: "cancel" },
            {
               ...
               onPress: async () => {
                  try {
                     ...
                     const remainingIncomes = incomeSources.filter((source) => source.id !== id);
                     ...
                     getFinancialSummary()
                        ...
                        .catch((err) => {
                           console.error("Failed to update financial summary after deletion:", err);
                        });
                  } catch (error) {
   ...
   const renderRightActions = (progress: any, dragX: any, id: number) => {
      ...
      return (
         <RectButton style={[styles.deleteButton, { backgroundColor: themeColors.red }]} onPress={() => handleDeleteIncome(id)}>
      ...
      <Swipeable
         ...
         <TouchableOpacity
            ...
            <View style={styles.incomeDetails}>
               <Text style={[styles.incomeTitle, { color: themeColors.card_title }]}>{source?.title || "Untitled"}</Text>
               <Text style={[styles.incomeSubtext, { color: themeColors.card_title }]}>
                  {source?.type === "Periodic" ? t("periodic") : t("oneTime")} • {source?.paymentMethod || t("income.unknownMethod")}
               </Text>
            </View>
            <Text style={[styles.incomeAmount, { color: themeColors.green }]}>${typeof source?.amount === "number" ? source.amount : "0"}</Text>
         </TouchableOpacity>
      </Swipeable>
   );
   ...
   const getMostRecentIncomes = (sources: IncomeSource[], count: number = 2): IncomeSource[] => {
   ...
   const renderFooter = () => {
      ...
      return (
         <View style={styles.footerLoader}>
            ...
            <Text style={{ color: themeColors.card_title, marginLeft: 8 }}>Loading more...</Text>
         </View>
      );
   };
   ...
   return (
      <View style={[styles.container, { backgroundColor: themeColors.page_background }]}>
         ...
         <View style={styles.content}>
            ...
            <View
               ...
               <View style={styles.cardHeader}>
                  <Text style={[styles.cardTitle, { color: themeColors.card_title }]}>{t("incomeSources")}</Text>
                  <TouchableOpacity
                     style={[
                        styles.addButton,
                        {
         ...
         <AddIncomeModal visible={isAddModalVisible} onClose={() => setIsAddModalVisible(false)} onAdd={handleAddIncome} isDarkMode={isDarkMode} />
      </View>
   );
};
