      ...
      <View style={styles.cardWrapper}>
         <View style={styles.container}>
                  ...
                  <View style={styles.legendContainer}>
                     {pieData.map((item, index) => (
                        <View key={index} style={styles.legendItem}>
                           ...
                           <Text style={[styles.legendText, { color: themeColors.card_title }]}>
                              {t(`categories.${item.name.toLowerCase().replace(/\s+/g, "_")}`)}
                           </Text>
                        </View>
                     ))}
                  </View>
                  <Text style={[styles.flipHint, { color: themeColors.card_description }]}>
                     {t("dashboard.tapToSeeMore") || "Tap to see all categories"}
                  </Text>
               </TouchableOpacity>
            </Animated.View>
            ...
            >
               <TouchableOpacity 
                  onPress={handleFlip} 
                  style={styles.backHeaderTouch}
               >
                  <Text style={[styles.title, { color: themeColors.card_title }]}>{t("dashboard.allSpendings") || "All Spendings"}</Text>
               </TouchableOpacity>
               ...
               >
                  {sortedData.map((item, index) => (
                     ...
                     >
                        ...
                        <Text style={[styles.categoryCardName, { color: themeColors.card_title }]} numberOfLines={1}>
                           {t(`categories.${item.name.toLowerCase().replace(/\s+/g, "_")}`)}
                        </Text>
                        <Text style={[styles.categoryCardAmount, { color: themeColors.card_title }]}>
                           {item.amount.toLocaleString()}
                        </Text>
                        <View style={[styles.rankBadge, { backgroundColor: isDarkMode ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.05)" }]}>
                           <Text style={[styles.rankText, { color: themeColors.card_title }]}>#{index + 1}</Text>
                        </View>
                     </View>
                  ))}
               </ScrollView>
               ...
               >
                  <Text style={[styles.flipHint, { color: themeColors.card_description }]}>
                     {t("dashboard.tapToSeeChart") || "Tap to return to chart"}
                  </Text>
               </TouchableOpacity>
            </Animated.View>
         </View>
